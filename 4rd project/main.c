...
void *writeMatrix2dToFile(int iter, DoubleMatrix2D *m){ 
  double value;
  FILE *f_temp;
  ...
  if (iter % periodoS != 0 ){
    ...
    for (int i=0; i<m->n_l; i++){
      for (int j=0; j<m->n_c; j++){
        value = dm2dGetEntry(m, i, j);
        fprintf(f_temp, " %8.4f", value);
      }
    fprintf(f_temp, "\n");
    }
  }
...
double dualBarrierWait (DualBarrierWithMax* b, int current, double localmax, int iter, DoubleMatrix2D *m) { /* --- adicionei iter como argumento */
...
void *tarefa_trabalhadora(void *args) {
  ...
  do {
    ...
    global_delta = dualBarrierWait(dual_barrier, atual, max_delta, iter, matrix_copies[prox]);
...
int main (int argc, char** argv) {
  ...
  periodoS = parse_integer_or_exit(argv[9], "periodoS", 0);
  fichS = fopen(argv[10], "r+");
  f = argv[10];



  fprintf(stderr, "\nArgumentos:\n"
   " N=%d tEsq=%.1f tSup=%.1f tDir=%.1f tInf=%.1f iter=%d trab=%d maxD=%.1f, periodoS=%d \n",
   N, tEsq, tSup, tDir, tInf, iter, trab, maxD, periodoS);
  ...
  /* --- Alterei aqui ---*/ 
  if (fichS != NULL ) {  //check if the file is empty or not.
    matrix_copies[0] = readMatrix2dFromFile(fichS, matrix_copies[0]->n_l, matrix_copies[0]->n_c); 
